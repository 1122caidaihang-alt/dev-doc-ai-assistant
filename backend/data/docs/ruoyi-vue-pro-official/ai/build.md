...
> 来源: https://doc.iocoder.cn/ai/build
...
进度说明：
 ...
 (opens new window) 仓库的 `master-jdk17` 分支
 
注意！仅支持 JDK 17/21 使用，因为基于 Spring AI
 
 (opens new window) 实现，它基于 Spring Boot 3.X 构建，所以最低要求 JDK 17！！！
...
① 修改根目录的 `pom.xml`
 ...
 (opens new window) 文件，取消 `yudao-module-ai` 模块的注释。如下图所示：
 

 
② 修改 `yudao-server` 目录的 `pom.xml`
 
 (opens new window) 文件，引入 `yudao-module-ai` 模块。如下图所示：
...
目前 AI 系统下的功能，是基于 LLM 大模型
 
 (opens new window)，从而实现对话、绘图等一系列功能。
