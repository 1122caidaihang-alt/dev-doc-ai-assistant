...
> 来源: https://doc.iocoder.cn/report

- [](/)
 - 开发指南
- 大屏手册
 [芋道源码](https://www.iocoder.cn) [2022-07-29](javascript:;)   目录  
# 报表设计器
...
在项目中，通过集成市面上的报表引擎，实现了报表设计器的能力。目前使用如下：
  是否集成 是否开源 JimuReport
 
 (opens new window) 已集成 不开源 AJ-Report
 
 (opens new window) 集成中 开源 UReport2
 
 (opens new window) 不集成 开源 
...
下载 `jimureport.mysql5.7.create.sql`
 
 (opens new window) 脚本，并导入数据库，初始化 JimuReport 相关的表结构和数据。如果你是 Oracle、PostgreSQL 等其它数据库，需要自己使用 Navicat 进行转换。
...
② 前端：在 `@/views/report/jmreport/index.vue`
