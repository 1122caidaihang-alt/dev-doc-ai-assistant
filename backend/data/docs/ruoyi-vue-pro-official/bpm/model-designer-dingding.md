...
> 来源: https://doc.iocoder.cn/bpm/model-designer-dingding
...
它的实现原理比较简单，分成 2 阶段：
 ...
 - 流程「设计」阶段：通过拖拽组件，配置节点属性，生成 JSON 格式的流程定义数据，保存到数据库，可见前端 SimpleProcessDesignerV2
 
 (opens new window) 组件
 - 流程「保存」阶段：后端将 JSON 格式的流程定义数据，转换成 BPMN XML 格式，保存到 Flowable 引擎，可见后端 SimpleModelUtils
...
ps：再开始之前，希望你已经阅读过 [《11.流程设计器（BPMN）》](/bpm/model-designer-bpmn/) 文档，相同的内容，就不再赘述。
