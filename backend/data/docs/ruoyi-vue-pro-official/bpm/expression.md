...
> 来源: https://doc.iocoder.cn/bpm/expression
...
Flowable 使用 UEL 进行表达式解析。UEL 代表 Unified Expression Language
 
 (opens new window)，是 EE6 规范的一部分。例如说：
...
BpmTaskAssignStartUserExpression
 
 (opens new window)，分配给发起人审批的 Expression 流程表达式，需要声明成 Spring Bean，代码如下：
...
BpmTaskAssignLeaderExpression
 
 (opens new window)，分配给发起人的指定 `level` 级别 Leader 审批的 Expression 流程表达式，需要声明成 Spring Bean，代码如下：
