...
> 来源: https://doc.iocoder.cn/report/screen

- [](/)
 - 开发指南
- 大屏手册
 [芋道源码](https://www.iocoder.cn) [2023-02-07](javascript:;)   目录  
# 大屏设计器
...
在项目中，通过集成市面上的报表引擎，实现了大屏设计器的能力。目前使用如下：
 ...
 (opens new window) 已经 开源 Go-View
 
 (opens new window) 已集成 开源 JimuReport
 
 (opens new window) 已集成 不开源 
## # 1. 功能开启
...
① 修改根目录的 `pom.xml`
 ...
 (opens new window) 文件，取消 `yudao-module-report` 模块的注释。
 

 
② 修改 `yudao-server` 目录的 `pom.xml`
 
 (opens new window) 文件，引入 `yudao-module-report` 模块。如下图所示：
...
① 克隆 yudao-ui-go-view
 ...
 (opens new window) 项目，执行如下命令进行启动：
...
② 启动完成后，浏览器会自动打开 http://127.0.0.1:3000
...
① 后端：Go-View 的后端代码，主要在 `go-view`
 
 (opens new window) 包下实现。
 
② 前端：在 `@/views/report/go-view`
 
 (opens new window) 文件，通过 IFrame 嵌入 Go-View 界面。
...
① 后端：在 `jmreport`
 ...
 (opens new window) 包下，进行 JimuReport 的集成。
 
② 前端：在 `@/views/report/jmreport/bi.vue`
 
 (opens new window) 文件，通过 IFrame 嵌入 JimuReport 界面。
