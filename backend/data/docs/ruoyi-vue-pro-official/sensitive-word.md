...
> 来源: https://doc.iocoder.cn/sensitive-word

- [](/)
 - 开发指南
- 系统手册
 [芋道源码](https://www.iocoder.cn) [2022-12-31](javascript:;)   目录  
# 敏感词
...
如果你系统里需要，可以参考 9a31613（后端）
 
 (opens new window)、0925253（前端）
...
敏感词采用 前缀树
 ...
 (opens new window) 算法，，核心代码见 SimpleTrie
...
SensitiveWordApi
 ...
 (opens new window) 提供了敏感词的 API 接口，可以在任意地方使用。方法如下：
...
public interface SensitiveWordApi {
     ...
     * @return 不合法的敏感词数组
     */
    ListString> validateText(String text, ListString> tags);
     ...
     * @return 是否包含
     */
    boolean isTextValid(String text, ListString> tags);
