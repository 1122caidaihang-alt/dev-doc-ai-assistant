...
> 来源: https://doc.iocoder.cn/project-rename
...
项目提供了 ProjectReactor
 
 (opens new window) 程序，支持一键改包，包括 Maven 的 `groupId`、`artifactId`、Java 的根 `package`、前端的 `title`、数据库的 SQL 配置、应用的 `application.yaml` 配置文件等等。效果如下图所示：
...
这里，也有球友分享了 https://t.zsxq.com/MovSn
 
 (opens new window) 合并策略，大家可以参考下。
...
如果启动过程中，碰到 Redis 报错，可见 https://t.zsxq.com/R3iAM
