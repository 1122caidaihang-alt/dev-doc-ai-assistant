...
> 来源: https://doc.iocoder.cn/redis-cache
...
`yudao-spring-boot-starter-redis`
 ...
 (opens new window) 技术组件，使用 Redis 实现缓存的功能，它有 2 种使用方式：
...
如果你未学习过 Spring Data Redis 框架，可以后续阅读 《芋道 Spring Boot Redis 入门》
...
① 在 `application-local.yaml`
 ...
 (opens new window) 配置文件中，通过 `spring.redis` 配置项，设置 Redis 的配置。如下图所示：
 

 
② 在 YudaoRedisAutoConfiguration
 
 (opens new window) 配置类，设置使用 JSON 序列化 value 值。如下图所示：
...
dependency>
    ...
    artifactId>yudao-spring-boot-starter-redisartifactId>
dependency>
...
新建 OAuth2AccessTokenDO
 
 (opens new window) 类，访问令牌 Access Token 类。代码如下：
...
在 `yudao-module-system` 模块的 RedisKeyConstants
 
 (opens new window) 类中，新建 OAuth2AccessTokenDO 对应的 Redis Key 定义 `OAUTH2_ACCESS_TOKEN`。如下图所示：
...
新建 OAuth2AccessTokenRedisDAO
 
 (opens new window) 类，是 OAuth2AccessTokenDO 的 RedisDAO 实现。代码如下：
...
在 OAuth2TokenServiceImpl
 
 (opens new window) 中，只要注入 OAuth2AccessTokenRedisDAO Bean，非常简洁干净的进行 OAuth2AccessTokenDO 的缓存操作，无需关心具体的实现。代码如下：
...
如果你未学习过 Spring Cache 框架，可以后续阅读 《芋道 Spring Boot Cache 入门》
...
`@Cacheable`
 ...
 (opens new window) 注解：添加在方法上，缓存方法的执行结果。执行过程如下：
...
`@CachePut`
 ...
 (opens new window) 注解，添加在方法上，缓存方法的执行结果。不同于 `@Cacheable` 注解，它的执行过程如下：
...
`@CacheEvict`
 ...
 (opens new window) 注解，添加在方法上，删除缓存。
 
不使用 `allEntries` 属性，但是想批量删除一些缓存，怎么办？
 
可参考 https://t.zsxq.com/phOrM
 
 (opens new window) 帖子，手动删除一些。
::
...
在 RoleServiceImpl
 ...
 (opens new window) 中，使用 Spring Cache 实现了 Role 角色缓存，采用【被动读】的方案。原因是：
...
如果你未学习过 MySQL 与 Redis 一致性的问题，可以后续阅读 《Redis 与 MySQL 双写一致性如何保证？ 》
...
实现的原来，参考 《Spring @Cacheable 扩展支持自定义过期时间 》
 
 (opens new window) 文章。
 
## # 3. Redis 监控
 
`yudao-module-infra` 的 `redis`
 
 (opens new window) 模块，提供了 Redis 监控的功能。
