...
> 来源: https://doc.iocoder.cn/mp/user
...
本章节，讲解公众号粉丝的相关内容，包括关注、取消关注等等，对应 《微信公众号官方文档 —— 获取用户列表》
...
对应后端的 MpUserServiceImpl
 
 (opens new window) 的 `syncUser` 方法。
 
## # 4. 关注 SubscribeHandler
 
用户关注公众号时，会触发 SubscribeHandler
 
 (opens new window) 处理器，新增或修改 `mp_user` 粉丝信息。
...
用户取消关注公众号时，会触发 UnsubscribeHandler
 
 (opens new window) 处理器，标记 `mp_user` 粉丝信息为取消关注，设置 `subscribe_status` 字段为 0。
