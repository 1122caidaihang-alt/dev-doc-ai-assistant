...
> 来源: https://doc.iocoder.cn/migrate-module

- [](/)
 - 开发指南
- 萌新必读
 [芋道源码](https://www.iocoder.cn) [2024-05-01](javascript:;)   目录  
# 迁移模块（适合新项目）
...
系统分成两个版本：
 版本 JDK 8 + Spring Boot 2.7 JDK 17/21 + Spring Boot 3.5 【完整版】ruoyi-vue-pro
 
 (opens new window) `master`
 
 (opens new window) 分支 `master-jdk17`
 
 (opens new window) 分支 【精简版】yudao-boot-mini
 
 (opens new window) `master`
 
 (opens new window) 分支 `master-jdk17`
 
 (opens new window) 分支 - 【完整版】：包括系统功能、基础设施、会员中心、数据报表、工作流程、商城系统、微信公众号、CRM、ERP、WMS、MES、IM 即时通讯、AI 大模型、IoT 物联网等功能
...
后端代码迁移，指的是将【完整版】的代码，复制张贴到【精简版】。
 项目 说明 `yudao-module-member` 会员中心的 Module 模块 `yudao-module-pay` 支付系统的 Module 模块 `yudao-module-mall` 商城系统的 Module 模块 `yudao-module-erp` ERP 系统的 Module 模块 `yudao-module-crm` CRM 系统的 Module 模块 `yudao-module-mp` 微信公众号的 Module 模块 `yudao-module-report` 大屏报表 Module 模块 `yudao-module-mes` MES 系统的 Module 模块 `yudao-module-wms` WMS 系统的 Module 模块 `yudao-module-im` IM 即时通讯的 Module 模块 `yudao-module-ai` AI 大模型的 Module 模块 `yudao-module-iot` IoT 物联网的 Module 模块 
### # 1.1 迁移步骤
...
如果碰到 `remaining.ts` 路由报错，可见 https://t.zsxq.com/69GWJ
