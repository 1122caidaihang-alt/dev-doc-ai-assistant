...
> 来源: https://doc.iocoder.cn/bpm

- [](/)
 - 开发指南
- 工作流手册
 [芋道源码](https://www.iocoder.cn) [2024-03-24](javascript:;)   目录  
# 功能开启
...
① 修改根目录的 `pom.xml`
 ...
 (opens new window) 文件，取消 `yudao-module-bpm` 模块的注释。
 

 
② 第二步，修改 `yudao-server` 的 `pom.xml`
 
 (opens new window) 文件，取消 `yudao-module-bpm` 依赖的注释，并进行 IDEA 的 Maven 刷新。
...
② 问题：如果启动中报 MySQL “Specified key was too long; max key length is 1000 bytes”
...
解决方案：参见 https://www.cnblogs.com/sowler/p/17195427.html
 
 (opens new window) 帖子，MySQL 不区分大小写导致的，需要调整为区分。
