...
> 来源: https://doc.iocoder.cn/new-feature
...
所以这种重复性的代码，项目提供了 codegen
 
 (opens new window) 代码生成器，只需要在数据库中设计好表结构，就可以一键生成前后端代码 + 单元测试 + Swagger 接口文档 + Validator 参数校验。
...
可见 CodegenBuilder
 ...
 (opens new window) 类，自动解析数据库的表结构，生成默认的配置。
...
字段信息
 ...
 前端项目 前端类型 yudao-ui-admin-vue2 Vue2 Element UI 标准模版 yudao-ui-admin-vue3 Vue3 Element Plus 标准模版 yudao-ui-admin-vben（antd） Vben5.0 Ant Design Schema 模版 yudao-ui-admin-vben（ele） Vben5.0 Element Plus Schema 模版 
生成信息
...
可见 CodegenEngine
 ...
 (opens new window) 类，基于 Velocity 模板引擎，生成具体代码。模板文件，可见 `resources/codegen`
...
答：星球里 https://t.zsxq.com/14Fc743WH
 
 (opens new window) 进行了 VO 拆分的讨论，总体大家倾向拆分成三个 VO 类，因为这样更加清晰，而且也不会增加太多的维护工作量。
...
类似 https://t.zsxq.com/owJzU
 
 (opens new window) 的问题。
...
参见 https://t.zsxq.com/pQypk
 
 (opens new window) 帖子改造。
 
## # 666. 社区贡献相关
 - 《Pull Request：允许同时生成管理端代码和用户 APP 代码》
 
 (opens new window)
 - 《Pull Request：支持 businessName 包含子目录》
 
 (opens new window)
 - 《Pull Request：快速生成代码到指定项目目录》：后端代码
 
 (opens new window)、前端代码
 
 (opens new window)
 - 《Pull Request：代码生成预览时检查类名称是否重复》：后端代码
