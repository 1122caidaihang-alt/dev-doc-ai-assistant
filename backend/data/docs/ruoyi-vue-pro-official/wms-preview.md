...
> 来源: https://doc.iocoder.cn/wms-preview

- [](/)
 - 开发指南
- WMS手册
 [芋道源码](https://www.iocoder.cn) [2026-05-15](javascript:;)   目录  
# WMS 演示
...
### # 1.1 WMS 管理后台
 ...
 (opens new window) 的 `wms` 目录，基于 Vue3 + Element Plus 实现
...
支持 Spring Boot 单体、Spring Cloud 微服务架构
 ...
 (opens new window) 的 `yudao-module-wms` 模块
 - 微服务仓库： https://github.com/YunaiV/yudao-cloud
 
 (opens new window) 的 `yudao-module-wms` 服务
...
WMS 共 **16** 张表，覆盖物料主数据、库存台账与流水、出入库 / 移库 / 盘库四类业务单据，表名统一以 `wms_` 为前缀：
 ...
 模块 子前缀 表数量 核心职责 基础数据 `wms_warehouse` / `wms_item*` / `wms_merchant` 6 仓库、商品体系（分类 / 品牌 / 商品 / SKU）、往来企业 库存管理 `wms_inventory*` 2 当前库存数量记录（按 `仓库 + SKU` 聚合）+ 库存变更流水 入库单 `wms_receipt_order*` 2 生产 / 采购 / 退货 / 归还 四类入库，共主表 + 明细 出库单 `wms_shipment_order*` 2 退货 / 销售 / 生产 三类出库，共主表 + 明细 移库单 `wms_movement_order*` 2 同公司内不同仓库间转移物料（主表 + 明细） 盘库单 `wms_check_order*` 2 实物盘点后调整账面库存差异（主表 + 明细） 
### # 5.1 基础数据
