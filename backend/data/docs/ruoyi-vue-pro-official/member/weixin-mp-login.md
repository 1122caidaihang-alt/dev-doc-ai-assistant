...
> 来源: https://doc.iocoder.cn/member/weixin-mp-login
...
② 下载 微信开发者工具
 ...
 (opens new window)，并进行安装。安装后，选择「公众号网页项目」。如下图所示：
 

 
### # 2.2 微信 JSSDK
 
访问 `http://127.0.0.1:3000/` 地址（其它地址也可以），它会触发 微信 JSSDK
 
 (opens new window) 初始化的逻辑，对应前端 `sheep/libs/sdk-h5-weixin.js` 文件的 `#init(...)` 方法中。
