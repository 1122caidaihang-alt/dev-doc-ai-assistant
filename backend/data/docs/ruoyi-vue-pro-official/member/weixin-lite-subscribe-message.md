...
> 来源: https://doc.iocoder.cn/member/weixin-lite-subscribe-message

- [](/)
 - 开发指南
- 会员手册
 [芋道源码](https://www.iocoder.cn) [2024-08-01](javascript:;)   目录  
# 微信小程序订阅消息
...
本文是 [《三方登录》](/social-user/) 的延伸，讲解 `yudao-mall-uniapp`
 
 (opens new window) 商城小程序如何实现微信 **小程序** 订阅消息的功能。对应的官方文档如下：
 - 《【开发指南】订阅消息》
...
在后端 发送订阅消息
 ...
 (opens new window) 时，需要传递 `miniprogram_state` 参数，用于区分跳转小程序类型：
 - `developer` 为开发版
 - `trial` 为体验版
 - `formal` 为正式版
...
目的是，uni-app 在调用 `wx.requestSubscribeMessage(Object object)`
 
 (opens new window) 发起订阅消息时，需要传递 `tmplIds` 消息模版的编号。
...
它的内部，通过调用项目的 SocialClientApi 的 `#sendWxaSubscribeMessage(...)` 方法，从而调用微信的 发送订阅消息
 
 (opens new window) 接口，发送订阅消息的请求。
