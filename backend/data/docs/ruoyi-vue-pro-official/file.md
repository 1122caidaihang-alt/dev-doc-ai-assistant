...
> 来源: https://doc.iocoder.cn/file

- [](/)
 - 开发指南
- 后端手册
 [芋道源码](https://www.iocoder.cn) [2022-03-17](javascript:;)   目录  
# 文件存储（上传下载）
...
技术选型？
 - 优先，✔ 推荐方案 1。如果无法使用云服务，可以自己搭建一个 MinIO 服务。参见 《芋道 Spring Boot 对象存储 MinIO 入门 》
...
可见 https://t.zsxq.com/wKmMW
 
 (opens new window) 帖子，有 2 种解决方案。
 
比较推荐的方式，是把“是否 Path Style”设置为 `true` 启用，原因是 https://www.inlighting.org/archives/aws-s3-virtual-hosted-style-vs-path-style
...
FileController
 ...
 (opens new window) 提供了 `/admin-api/infra/file/upload` RESTful API，用于前端直接上传文件。
...
public CommonResultString> uploadFile(FileUploadReqVO uploadReqVO) throws Exception {
...
`yudao-module-infra` 的 FileApi
 
 (opens new window) 提供了 `#createFile(...)` 方法，用于后端需要上传文件的逻辑。
...
② 当存储器是【数据库】【本地磁盘】等时，它们只支持存储，所以需要 FileController
 
 (opens new window) 提供的 `/admin-api/infra/file/{configId}/get/{path}` RESTful API，读取文件内容后返回。
...
在 `yudao-module-infra` 模块中，它的 `framework/file`
 
 (opens new window) 包下，定义了 FileClient
 
 (opens new window) 接口，抽象了文件客户端的方法。代码如下所示：
...
做的不错的云存储服务，都是兼容 S3 协议的。如何获取对应的 S3 配置，艿艿整理到了 S3FileClientConfig
 
 (opens new window) 配置类。
...
## # 666. 社区贡献的文件相关
 ...
 (opens new window)、#778
 
 (opens new window)
 - 支持 Hbase 文件存储格式
 
 (opens new window)
 - 《Pull Request：增加图片自动压缩》
 
 (opens new window)
 - 《Pull Request：实现流式文件上传与下载，支持大文件上传》
 
 (opens new window)
 - 《Pull Request：upload first file item from clipboard》
 
 (opens new window)
 - 《Pull Request：增加 minio 的专属客户端、使用 minio 客户端实现断点续传的功能》
 
 (opens new window)
 - S3 文件客户端支持基础路径，可作为腾讯云 COS 桶下的自定义虚拟根路径：#1502
 
 (opens new window)、#855
