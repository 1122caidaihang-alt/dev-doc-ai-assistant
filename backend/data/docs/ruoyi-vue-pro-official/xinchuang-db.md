...
> 来源: https://doc.iocoder.cn/xinchuang-db

- [](/)
 - 开发指南
- 萌新必读
 [芋道源码](https://www.iocoder.cn) [2023-11-06](javascript:;)   目录  
# 国产信创数据库（DM 达梦、大金、OpenGauss）
...
① 访问 https://eco.dameng.com/download/
 
 (opens new window) 地址，点击“开发版 (X86平台)”选项，进行下载。
...
① 访问 https://eco.dameng.com/download/
 ...
 (opens new window) 地址，点击“Docker 镜像”选项，进行下载。
...
使用 DM 管理工具
 ...
 (opens new window) 进行数据的导入。如果你不是很了解，已可以看看文档：
...
实现思路非常简单：
 - Quartz 初始化使用 `tables_oracle`
 
 (opens new window) 脚本，去掉前面的 `delete` 和 `drop` 部分
...
① 访问 https://www.kingbase.com.cn/xzzx/index.htm
 
 (opens new window) 地址，点击左侧的“软件版本 -> 数据库” 菜单，再点击“docker镜像”选项，进行下载：
 - x86_64 版本
 
 (opens new window) 【Windows 选择这个】
 - aarch64 版本
 
 (opens new window) 【MacBook Apple Silicon 选择这个】
...
① 参考 《用 JetBrains DataGrip 连接 Huawei openGauss》
 
 (opens new window) 文档，连接 openGauss 数据库。
