...
> 来源: https://doc.iocoder.cn/vue3/dev-spec
...
基于 Virtualized Table 虚拟化表格
 
 (opens new window) 实现，解决一屏里超过 1000 条数据记录时，就会出现卡顿等性能问题。
...
在 `@views`
 ...
 (opens new window) 目录下，每个模块对应一个目录，它的所有功能的 `.vue` 都放在该目录里。
...
在 `@/api`
 ...
 (opens new window) 目录下，每个模块对应一个 `index.ts` API 文件。
...
`/src/config/axios/index.ts`
 ...
 (opens new window) 基于 axios
 
 (opens new window) 封装，统一处理 GET、POST 方法的请求参数、请求头，以及错误提示信息等。
...
service.interceptors.response.use(
    async (response: AxiosResponseany>) => {
        const { data } = response
...
在 `@/components`
 ...
 (opens new window) 目录下，实现**全局**组件，被所有模块所公用。
...
① 在 `@/styles`
 ...
 (opens new window) 目录下，实现**全局**样式，被所有页面所公用。
...
可参考 《vue-element-plus-admin —— 项目规范》
