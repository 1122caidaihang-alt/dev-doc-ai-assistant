...
> 来源: https://doc.iocoder.cn/pay/refund-demo
...
在 `yudao-module-pay` 模块的 `demo`
 ...
 (opens new window) 模块，我们提供了一个 **退款** 接入的示例，它已支持支付宝、微信的所有退款方式。
...
② 【后端】在 `demo` 模块的退款逻辑中，需要调用 PayRefundApi 的 `#createRefund(...)`
 
 (opens new window) 方法，创建退款单。如下图所示：
...
在 `demo` 模块所在的 `yudao-module-xx` 模块，实现一个支付回调的接口，提供给支付【中心】回调。对应的代码在 PayDemoOrderController 的 `#updateDemoOrderRefunded(...)`
