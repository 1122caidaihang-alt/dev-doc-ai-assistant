...
> 来源: https://doc.iocoder.cn/pay/wallet

- [](/)
 - 开发指南
- 支付手册
 [芋道源码](https://www.iocoder.cn) [2024-09-18](javascript:;)   目录  
# 钱包充值、支付、退款
...
在 `yudao-module-pay` 模块的 `wallet`
 
 (opens new window) 模块，实现了钱包，包括了如下功能：
...
另外，钱包也有对应的的支付、退款回调地址，如下图所示：
 ...
 - 支付回调地址：由 PayWalletRechargeController 的 `#updateWalletRechargerPaid(...)`
 
 (opens new window) 方法实现
 - 退款回调地址：由 PayWalletRefundController 的 `#updateWalletRechargeRefunded(...)`
 
 (opens new window) 方法实现
 
友情提示：上述两个回调地址的逻辑，可以等后续再看，不影响充值流程的理解。
 
② 在商城 uni-app 端，访问 http://127.0.0.1:3000/pages/pay/recharge
 
 (opens new window) 地址，进入 [充值] 界面，对应 <yudao-mall-uniapp/pages/pay/recharge.vue> 界面。如下图所示：
