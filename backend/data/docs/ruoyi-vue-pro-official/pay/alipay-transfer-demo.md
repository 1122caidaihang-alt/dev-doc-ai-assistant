...
> 来源: https://doc.iocoder.cn/pay/alipay-transfer-demo
...
在 `yudao-module-pay` 模块的 `demo`
 ...
 (opens new window) 模块，我们提供了一个 **转账** 接入的示例（PayDemoWithdrawController）。
...
② 【后端】在 `demo` 模块的转账逻辑中，需要调用 PayTransferApi 的 `#createTransfer(...)`
 
 (opens new window) 方法，创建转账单。如下图所示：
...
在 `demo` 模块所在的 `yudao-module-xx` 模块，实现一个转账回调的接口，提供给支付【中心】回调。对应的代码在 PayDemoWithdrawController 的 `#updateDemoWithdrawTransferred(...)`
