...
> 来源: https://doc.iocoder.cn/http-sign

- [](/)
 - 开发指南
- 后端手册
 [芋道源码](https://www.iocoder.cn) [2024-06-04](javascript:;)   目录  
# HTTP 接口签名（防篡改）
  
`yudao-spring-boot-starter-protection`
 
 (opens new window) 技术组件，由它的 `signature`
 
 (opens new window) 包，提供 HTTP 接口签名特性，提高安全性。
...
DigestUtil.sha256Hex(serverSignatureString);
    ...
    private String buildSignatureString(ApiSignature signature, HttpServletRequest request, String appSecret) {
        SortedMapString, String> parameterMap = getRequestParameterMap(request); // 请求头
        SortedMapString, String> headerMap = getRequestHeaderMap(signature, request); // 请求参数
